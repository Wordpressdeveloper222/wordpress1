...
(function( $ ) {
    ...
    redux.field_objects.custom_sites = redux.field_objects.custom_sites || {};
    ...
    redux.field_objects.custom_sites.init = function( selector ) {

        if ( !selector ) {
            selector = $( document ).find( ".redux-group-tab:visible" ).find( '.redux-container-custom_sites:visible' );
        }
        ...
        $( selector ).each(
            function() {
                ...
                if ( parent.hasClass( 'redux-container-custom_sites' ) ) {
                ...
                el.find( '.redux-slides-remove' ).live(
                    'click', function() {
                        ...
                        var slideCount = $( this ).parents( '.redux-container-custom_sites:first' ).find( '.redux-slides-accordion-group' ).length;
                        ...
                        if ( slideCount > 1 ) {
                            ...
                            $( this ).parents( '.redux-container-custom_sites:first' ).find( '.redux-slides-accordion-group:last' ).find( '.redux-slides-header' ).text( content_new_title );
                        }
                    }
                );
                ...
                el.find( '.redux-slides-add' ).off('click').click(
                    function() {
                        ...
                        var slideCount = $( newSlide ).find( '.site-title' ).attr( "name" ).match( /[0-9]+(?!.*[0-9])/ );
