...
/*add_action( 'init', 'vlog' );
function vlog() {
  register_post_type( 'vlog',
    array(
      'labels' => array(
        'name' => __( 'Vlog' ),
        'singular_name' => __( 'Vlog' )
      ),
      'public' => true,
      'has_archive' => true,
	  'show_in_nav_menus' => true,
...
add_action( 'init', 'create_vlog_tax' );

function create_vlog_tax() {
	register_taxonomy(
        'vlogcat',
        'vlog',
        array(
            'label' => __( 'Vlog Category' ),
          'public' => true,
          'rewrite' => true,
            'hierarchical' => true,
			 'show_in_nav_menus' => true, 
...
/*function vlog_content() {
				$args_vlog = array( 'post_type' => 'vlog', 'posts_per_page' => 4 );		
				$the_query_vlog = new WP_Query( $args_vlog ); 
				 if ( $the_query_vlog->have_posts() ) :
				 ?><h3 class="title">latest</h3>
				<div class="row">
				<?php while ( $the_query_vlog->have_posts() ) : $the_query_vlog->the_post(); ?>
				<div class="col-sm-6 text-center">
				<?php the_post_thumbnail(array(280,360),array('class' => 'img-responsive')); ?>
			  ...
			  ?></div><?php 
				...
				echo $term_id = $categories->term_id; 
				echo '<ul>';
				...
				?>
					...
					<div class="cat_img"><img src="<?php echo $meta_image?>" /></div>
					<div class="cat_title"><a href="<?php echo $url;?>"><?php echo $category->name; ?></a></div>
					<div class="cat_des"><?php echo $category->description; ?></div>
					<div class="row">
				<?php
				}
...
add_shortcode( 'Vlog', 'vlog_content' );
...
?>
<?php
/* Parameters Builder Loading */
//require_once get_template_directory() . '/inc/parameters-builder.php';
...
function display_logged_in_users(){
	...
	if ( !empty( $logged_in_users ) ) {
		echo "<br/><strong>Logged in users:</strong>";
		echo '<ul class="user_list">';
			foreach ( $logged_in_users as $key => $value) {
					...
					?> 
					<li id ="<?php echo $user->ID;?>" onclick="getmessages(<?php echo $user->ID;?>)" class="user-de-tails"><?php echo get_avatar($user->ID); ?>
					<span class="u-name"><?php echo  $user->display_name?></span></li>
					<?php
				}
			}
		echo '</ul>';
